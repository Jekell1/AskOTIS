...
## Program Overview
`OPEN-GB-FILE` opens the global batch file, which stores global configuration and control records for the APIPAY batch process. This file is essential for accessing system-wide settings and parameters.

## Transaction Types Supported
- Not transaction-based; supports all operations requiring global batch file access.

## Input Parameters
- None directly; uses system configuration for file path.

## Output Fields
- None directly; makes the global batch file available for subsequent operations.

## Program Flow Diagrams
### High-Level Flow
```mermaid
graph TD
    A[Start] --> B[Assign GB file path]
    B --> C[Open GB file]
...
- Assigns the file path for the global batch file.
...
```mermaid
graph TD
    Input[System Config] --> Path[GB File Path]
...
## Integration Points
- Used by APIPAY and other routines requiring global configuration.

## File Dependencies
- Global batch file (see source for path)

## Call Graph of PERFORMed Paragraphs
```mermaid
graph TD
    OPEN-GB-FILE --> [Return to caller]
```
