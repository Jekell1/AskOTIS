...
## Program Overview
`OPEN-IT` opens the internal transaction file, which is used for recording and processing internal transactions in the APIPAY batch process. This file is required for reading and updating internal transaction data.

## Transaction Types Supported
- Not transaction-based; supports all operations requiring internal transaction file access.

## Input Parameters
- None directly; uses system configuration for file path.

## Output Fields
- None directly; makes the internal transaction file available for subsequent operations.

## Program Flow Diagrams
### High-Level Flow
```mermaid
graph TD
    A[Start] --> B[Assign IT file path]
    B --> C[Open IT file]
...
- Assigns the file path for the internal transaction file.
...
```mermaid
graph TD
    Input[System Config] --> Path[IT File Path]
...
## Integration Points
- Used by APIPAY and other routines requiring internal transaction data.

## File Dependencies
- Internal transaction file (see source for path)

## Call Graph of PERFORMed Paragraphs
```mermaid
graph TD
    OPEN-IT --> RETURN_TO_CALLER["Return to caller"]
```
