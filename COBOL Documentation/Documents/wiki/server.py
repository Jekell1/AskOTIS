...
This server is designed to run from the wiki directory and serve content from the parent Documents directory.
"""
...
def start_server(port=None, host=DEFAULT_HOST, auto_open=True):
    ...
    if not os.path.exists('index.html'):
        ...
        print("   Make sure you're running this script from the wiki directory")
        return False
    
    # Check if parent Documents directory exists
    docs_dir = os.path.join(os.path.dirname(script_dir))
    if not os.path.exists(docs_dir):
        print("❌ Error: Parent Documents directory not found")
    ...
    try:
        ...
        with socketserver.TCPServer((host, port), WikiHTTPRequestHandler) as httpd:
            server_url = f"http://{host}:{port}/index.html"
            ...
            print(f"   • Wiki Home:       {server_url}")
            print(f"   • Documentation:   Parent directory Markdown files")
            print(f"   • Diagrams:        Parent directory Diagrams/*.mmd")
            print("=" * 60)
            print("💡 Tips:")
            print("   • Press Ctrl+C to stop the server")
            print("   • Server automatically handles CORS and caching")
            print("   • All Markdown and Mermaid files are accessible via relative paths")
            print("=" * 60)
...
Examples:
  ...
  python server.py --port 8090             # Start on port 8090
  python server.py --no-browser            # Start without opening browser
  python server.py --host 0.0.0.0          # Allow external connections

Note: This server should be run from the wiki directory and will serve
content from the parent Documents directory via relative paths.
        """
    )
