#!/usr/bin/env python3
"""
PROGRAM FLOWS COMPLETION PROCESSOR
Dedicated processor to complete new_cobol_program_flows to 100%.
"""
...
def get_flows_status(config: Dict) -> Dict:
    """Get current status of program_flows index."""
...
def process_flows_batch(config: Dict, batch_size: int = 500) -> Dict:
    """Process a batch of program_flows records."""
    index_name = 'new_cobol_program_flows'
    ...
    print(f"   Processing {len(records)} program_flows records...")
    ...
    for i, record in enumerate(records):
        ...
        try:
            # Extract text from flow-specific fields
            text_parts = []
            
            # Priority fields for program flows
            fields_to_extract = [
                'flow_summary',
                'program_id', 
                'condensed_mermaid',
            ...
            if not text_parts:
                ...
                text_parts.append(f"Program flow for {program_id}")
...
def complete_program_flows():
    """Complete the program_flows index to 100%."""
    config = load_config()
    
    print("🎯 PROGRAM FLOWS COMPLETION PROCESSOR")
    print("=" * 60)
    print("Target: Complete new_cobol_program_flows to 100%")
    print(f"Azure Search: {config['search_endpoint']}")
    print(f"Azure OpenAI: {config['openai_endpoint']}")
    
    # Check initial status
    initial_status = get_flows_status(config)
    print(f"\n📊 INITIAL STATUS:")
    ...
    while True:
        batch_count += 1
        current_status = get_flows_status(config)
        ...
        result = process_flows_batch(config, batch_size)
    ...
    # Final status
    final_status = get_flows_status(config)
    
    print(f"\n" + "=" * 60)
    print("🎯 PROGRAM FLOWS COMPLETION SUMMARY")
    print("=" * 60)
