...
Create a new facts index `cobol-facts-v3` to pair with `cobol-paragraphs-v3`.
Schema builds on v2 (cobol-facts) expectations plus fields used by extract_posting_facts.py
and vector enablement pattern from patch_facts_index.

Fields:
  fact_id (key)
  program_id (filterable)
  action_role (filterable/facetable)
  posting_type (filterable/facetable)
  gating_cond (searchable)
  fact_confidence (double, filterable, sortable)
  sources (Collection(Edm.String))
  fact_text (searchable)  -- optional enriched natural language statement
  fact_vector (vector)    -- future embedding support
  has_vector (boolean)
