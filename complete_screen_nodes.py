#!/usr/bin/env python3
"""
SCREEN NODES COMPLETION PROCESSOR
Dedicated processor to complete new_cobol_screen_nodes to 100%.
"""
...
def get_screen_nodes_status(config: Dict) -> Dict:
    """Get current status of screen_nodes index."""
...
def process_screen_nodes_batch(config: Dict, batch_size: int = 500) -> Dict:
    """Process a batch of screen_nodes records."""
    index_name = 'new_cobol_screen_nodes'
    ...
    print(f"   Processing {len(records)} screen_nodes records...")
    ...
    for i, record in enumerate(records):
        ...
        try:
            # Extract text from screen-specific fields
            text_parts = []
            
            # Priority fields for screen nodes
            fields_to_extract = [
                'summary_text',
                'screen_name',
                'program_id',
            ...
            for field in fields_to_extract:
                ...
                if value and str(value).strip():
                    ...
                    # Handle JSON fields - extract meaningful content
                    if field.endswith('_json'):
                        try:
                            json_obj = json.loads(text_content)
                            if isinstance(json_obj, list) and json_obj:
                                # For lists, take first few items and summarize
                                sample_items = json_obj[:3]
                                text_content = f"{field}: {len(json_obj)} items - {json.dumps(sample_items, separators=(',', ':'))[:300]}"
                            elif isinstance(json_obj, dict):
                                # For dicts, extract key info
                                text_content = f"{field}: {json.dumps(json_obj, separators=(',', ':'))[:300]}"
                            else:
                                text_content = f"{field}: {str(json_obj)[:300]}"
                        except:
                            text_content = f"{field}: {text_content[:300]}"
                    else:
                        text_content = text_content[:400]
                    
                    text_parts.append(text_content)
            
            # Enhanced fallback for screen nodes
            if not text_parts:
                ...
                text_parts.append(f"Screen {screen_id} from program {program_id} with {field_count} fields and {action_count} actions")
            
            text = ' | '.join(text_parts)
            
            # Ensure minimum text length for better embeddings
            if len(text.strip()) < 20:
                screen_id = record.get('screen_id', 'unknown')
                program_id = record.get('program_id', 'unknown')
                text = f"Screen structure {screen_id} from COBOL program {program_id} with user interface elements"
...
def complete_screen_nodes():
    """Complete the screen_nodes index to 100%."""
    config = load_config()
    
    print("🎯 SCREEN NODES COMPLETION PROCESSOR")
    print("=" * 60)
    print("Target: Complete new_cobol_screen_nodes to 100%")
    print(f"Azure Search: {config['search_endpoint']}")
    print(f"Azure OpenAI: {config['openai_endpoint']}")
    
    # Check initial status
    initial_status = get_screen_nodes_status(config)
    print(f"\n📊 INITIAL STATUS:")
    ...
    while True:
        batch_count += 1
        current_status = get_screen_nodes_status(config)
        ...
        result = process_screen_nodes_batch(config, batch_size)
    ...
    # Final status
    final_status = get_screen_nodes_status(config)
    ...
    if final_status['coverage_percent'] >= 99.9:
        print(f"🎉 SUCCESS: Screen nodes index is complete!")
    else:
