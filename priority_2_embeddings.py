...
class Priority2EmbeddingGenerator:
    """
    Generates embeddings for Priority 2 indexes:
    4. new_cobol_data_items (267,769 docs) - Data structure search
    5. code-chunks (102,654 docs) - Legacy code chunks
    """
    ...
    def __init__(self):
        ...
        # Priority 2 index configurations
        self.priority_indexes = {
            'new_cobol_data_items': {
                'text_fields': ['full_clause', 'item_name', 'pic', 'path'],
                'vector_field': 'contentVector',
                'batch_size': 30,
                'key_field': 'item_id',
                'description': 'COBOL data structure definitions'
            },
            'code-chunks': {
                'text_fields': ['text'],
                'vector_field': 'contentVector', 
                'batch_size': 20,
                'key_field': 'chunk_id',
                'description': 'Legacy code chunks'
            }
        }
    ...
    def get_documents_without_embeddings(self, index_name, vector_field, batch_size=50):
        ...
        try:
            ...
            query = {
                "search": "*",
                "filter": f"not search.ismatch('*', '{vector_field}')",
                "top": batch_size,
                "select": "*"
            }
    ...
    def run_priority_2_embeddings(self, workers_per_index=3):
        """Run embedding generation for all Priority 2 indexes"""
        print(f"🚀 PRIORITY 2 EMBEDDING GENERATION")
        ...
        if total_pending == 0:
            print("🎉 All Priority 2 indexes already have embeddings!")
            return
...
if __name__ == "__main__":
    generator = Priority2EmbeddingGenerator()
    generator.run_priority_2_embeddings(workers_per_index=3)