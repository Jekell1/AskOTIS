...
```json
{
  ...
  "text_vector": [0.023, -0.045, ...],  // 3072 dimensions
  "has_vector": true
}
```
...
- Line-range filtering for transaction-specific queries
...
│  • Calls Azure OpenAI GPT-4 (gpt-4.1)                          │
...
def _handle_transaction_copybooks(query: str, max_results: int):
    ...
    code_chunks = search(filter=f"name eq '{program_name}.CBL'")
    ...
    if not copybooks:
        ...
        copybooks = semantic_search(copybook_meta, embedding, filter=f"program_id eq '{program_name}'")
...
**Features:**
- Stores last 30 conversation turns
...
| Question Type | Max Tokens | Approx Words | Rationale |
|---------------|------------|--------------|-----------|
...
**GPT-4 Context Window:** 128,000 tokens total
- **Input (prompt):** ~20,000-30,000 tokens (varies by retrieved docs)
- **Output (response):** 2,000-15,000 tokens (dynamic)
- **Leaves ~80,000 tokens buffer** for very large contexts
...
| **Chat API** (gpt-4.1) | 150,000 TPM | Typical: 10,000 TPM |
...
| **Index search** | 0.1-0.5s per index | Parallel queries |
| **Deterministic retrieval** | 1.0-2.0s | Multiple queries + fallback |
| **Memory retrieval** | 0.01s | In-memory lookup |
| **LLM generation** | 2.0-5.0s | Depends on max_tokens |
...
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │  function_app.py                                               │ │
│  │  • Singleton RAG instance (performance optimization)           │ │
...
│  (Standard S1)          │           │  (GPT-4.1 + Embeddings)      │
│                         │           │                              │
│  21 Indexes:            │           │  • gpt-4.1 (chat)           │
...
- **GPT-4.1** for natural language generation
